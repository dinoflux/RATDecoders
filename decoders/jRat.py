...
try:
    ...
    print >> sys.stderr, "[+] Couldn't Import Cipher, try 'sudo pip install pycrypto'"
...
def config(data):
    print >> sys.stderr, "[+] Extracting Data from Jar"
    enckey, conf = get_parts(data)
    if enckey == None:
        return
    print >> sys.stderr, "[+] Decoding Config with Key: {0}".format(enckey.encode('hex'))
    if len(enckey) == 16:
...
def get_parts(data):
    ...
    try:
        ...
        print >> sys.stderr, "[+] Dropped File is not Jar File starts with Hex Chars: {0}".format(data[:5].encode('hex'))
        return None, None
    if enckey and conf:
        return enckey, conf
